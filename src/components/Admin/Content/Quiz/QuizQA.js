...
function QuizQA () {
    ...
    const fetchQuizWithQA = async () => {
        ...
        if (res && res.EC === 0) {
            // chuyen base64 sang file object
            let newQA = []
            for (let i = 0; i< res.DT.qa.length; i++) {
                if (res.DT.qa[i].imageFile) {
                    res.DT.qa[i].imageName = `Question-${res.DT.qa[i].id}.png`
                    res.DT.qa[i].imageFile = await urltoFile(`data:image/png;base64,${res.DT.qa[i].imageFile}`, 
                                                        res.DT.qa[i].imageName,'image/png')
                }
                            ...
                            <div className='q-main mb-4' key={question.id}>
                                <div className='questions-content'>    
                                    <div className="form-floating desc">
                                        ...
                                        />
                                        <label htmlFor="floatingInput">{t('quizQA.question')} {index + 1} {t('quizQA.char')} {t('quizQA.description')}</label>
                                    </div>
                                    <div className='group-upload'>
                                        <label className='label-upload' htmlFor={`${question.id}`}>
                                            <RiImageAddFill className='icon'/>
                                            <span>{t('quizQA.image')}</span>
                                        </label>
                                        <input 
                                            id={`${question.id}`}
                                            type='file' 
                                        ...
                                        return (
                                            <div className='answers-content' key={answer.id}>
                                                ...
                                                <div className="form-floating answer-name">
                                                    ...
                                                    />
                                                    <label htmlFor={`checkbox-${answer.id}`}>{t('quizQA.answer')} {index + 1}</label>
                                                </div>
